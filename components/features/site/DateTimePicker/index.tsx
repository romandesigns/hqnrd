...
/**
 * NEW UTILITY:
 * Parses a time prop string in formats like "4:30 PM" or "4:30:00 PM"
 * and returns an object with hours, minutes, and seconds in 24-hour format.
 */
function parseTimeProp(
  time: string,
...
>(
  (
    ...
    const handleSelect = (newDay?: Date) => {
      ...
      if (time) {
        const parsed = parseTimeProp(time);
        if (parsed) {
          finalDate.setHours(parsed.hours, parsed.minutes, parsed.seconds, 0);
        }
      }
        ...
        <Popover>
          <PopoverTrigger asChild disabled={disabled}>
            <Button
              variant="outline"
              className={cn(
                "w-full justify-start bg-transparent text-left font-normal max-md:text-[0.82rem]",
            ...
            {granularity !== "day" && (
              <div className="border-t border-border p-3">
                <TimePicker
                  onChange={(value) => {
                    if (value && time) {
                      // Override the time from the TimePicker with the provided time prop.
